/* ***************************************************************************** */
/*    MIKA's ten key typing trainer  C++ source code                             */
/*                                                    Ver2.05.01 2024/12/6       */
...
char MIKA_file_name_seiseki[] = "mikaten.rec"; /* 成績ファイル名 読み込み用 */
char MIKA_file_name_seiseki2[] = "mikaten.rec"; /* 成績ファイル名 書き込み用 */
...
string	MIKA_mes0 = "●●●  Mika's Ten Key Trainer  ●●●";
string	MIKA_mesta = "●● Mika's Ten Key Trainer (%s) ●●";
string 	MIKA_mesi1 = "Press TAB key to try again";
string	MIKA_mesi2 = "Press ESC key to return to Menu";
string	MIKA_mesi3 = "Congratulations You have updated your speed record";
string	MIKA_abort_mes = "Press ESC key to cancel";
string	MIKA_return_mes = "Press ESC key to return to Menu";
string	MIKA_key_type_mes = "．．";
string 	MIKA_kugiri_mes="Press Enter key to separate Number";
string	MIKA_keymes1 = "Press Space key to hide Keyguide";
string	MIKA_keymes2 = "Press Space key to show Keyguide";
string	MIKA_keymes3 = "Please press Space key to hide Keyguide and try again";
string	MIKA_keymes4 = "Please press Space key to show Keyguide and try again";
string	MIKA_mest2 = "Practice item   Typing speed per minute         Date        Practice time";
...
string  MIKA_menu_mes_ss[] = { /* 成績ファイル書き込み メニュー項目 */
		"Position_Practice",
		"Random_Practice",
		"Decimal_Point",
		"Date"
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   SetMenu(hWnd, NULL); /* メニューバー非表示 */
	SetWindowText(hWnd,L"MIKATEN"); /* タイトル設定 */
...
void	procinit(void) /* 練習成績ファイル読み込み 練習日時取得 乱数初期化 */
{
	...
	string t0conv(long time,int flag) /* 練習時間秒をフォーマットを指定して文字列に変換 */
	{
		...
		t2=time%60; /* 分を計算 */
		t1=time/60; /* 時間を計算 */
		if(flag==0)	snprintf(a,20,"%5dhour%2dmin%2dsec",t1,t2,t3); /* 時分秒を文字列に変換 */
		else if(flag==1) snprintf(a,20,"%3dhour%2dmin%2dsec",t1,t2,t3);
		else snprintf(a,20,"%5dhour%2dmi%2dse",t1,t2,t3); /* 時分秒を文字列に変換 */
		return a;
	}
...
// j 表示位置横列番号
	{
		...
		{
 			cslcolor(g,MIKA_red);
			snprintf(type_mes,16,"Error count%3d",MIKA_type_err_count); /* エラーカウントメッセージ作成 */
			offset=0;
		}
		else if(flag==1)
		{
...
// flag=0 表示 flag=1 数値のみ消去 flag=2 メッセージと共に数値を消去
	{
		...
		if(flag==0)
		{
			...
			snprintf(type_mes,15,"Type count %3d",MIKA_type_count); /* 正解数メッセージ作成 */
			offset=0;
		}
		else if(flag==1)
		{
			cslcolor(g,MIKA_bk_color); /* フラグが1の時は数値のみ表示を消去 */
			snprintf(type_mes,15,"%3d",MIKA_type_count); /* 正解数メッセージ作成 */
			offset=11;
		}
		else
		{
...
//		System.out.printf("count=%d  MIKA_type_kind_no=%d\n",count,MIKA_type_kind_no);
		...
		snprintf(type_mes,19,"Practice count%4d",count); /* 練習回数メッセージ作成 */
...
// flag=0 表示 flag=1 消去 
	{
		...
		snprintf(type_mes,27,"It took%4.0fsec to finish",MIKA_type_speed_time); /* 表示メッセージ作成 */
...
//					dispbkchar(g,jj,ii,MIKA_random_scale); /* 文字の背景を赤色で表示 */ /* 2023/2/25 新コード */
	...
	void dispmaxspeedrecord(HDC g,int i1,int j1,int i2,int j2) /* ランダム練習の 最高入力速度と 達成日を表示 */
	{
			...
			snprintf(a,31,"Maximum Typing Speed%6.1f/min",MIKA_type_speed_record[MIKA_type_kind_no]); /* 最高速度メッセージ作成 */
			cslput(g,i1*16,j1*8,a); /* 最高速度メッセージ表示 */
			snprintf(b,16,"Date %s",MIKA_type_date_record[MIKA_type_kind_no].c_str()); /* 達成日メッセージ作成 */
	...
	void disptrain(HDC g,string mest) /* ランダム練習 実行画面の表示 */
	{
		...
		{
			dispmaxspeedrecord(g,3,22,3,53); /* 最高入力速度と達成日時を表示 */
		}
...
/* i 表示位置 j 表示個数 menu_mes 練習項目 r_speed 最高速度 r_date 達成日 r_time 累積練習時間 */
	...
	void dispseiseki(HDC g) /* 成績表示 */
	{
		...
		snprintf(aa,40,"Past Practice Time  %s",a.c_str()); /* 前回までの合計練習時間のメッセージ作成 */
...
//		aa=String.format("今回の練習時間　　　%s",a); /* 今回練習時間のメッセージを作成 */
		snprintf(aa,40,"Current Elapsed Time%s",a.c_str()); /* 前回までの合計練習時間のメッセージ作成 */
		cslcolor(g,MIKA_green); /* 表示色を緑色に設定 */
...
void dispstart(HDC g) /* 著作権表示 */
{
 ...
 //   cslclr(g); /* 画面クリア */
    ...
    cslmencenter(g, 5* 16 + 8, "Ｍｉｋａ'ｓ Ｔｅｎ Ｋｅｙ Ｔｒａｉｎｅｒ");
    cslcolor(g, MIKA_cyan); /* 表示色をシアンに設定 */
    cslmencenter(g, 7 * 16 + 8, "ＭＩＫＡＴＥＮ Ｖer２.０５．０１");
    cslcolor(g, MIKA_orange); /* 表示色をオレンジに設定 */
    cslmencenter(g, 9 * 16 + 6, "＜＜ Ten Key Practice ＞＞");
    cslcolor(g, MIKA_cyan); /* 表示色をシアンに設定 */
    cslmencenter(g, 14 * 16 - 8, "Copy right 1992/11/10  Jirou Imamura");
    cslput(g, 17 * 16, 38 * 8, "Hit any key to show Menu");
...
void menexe(HDC g,string menu_mes[], int menu_cord[6][2], int menu_function[], int sel_flag[], string menut)
{
		...
		int x;
		int y;
		string	mesi5="Press Number key to select Menu";
...
int wwseiseki(FILE *fp) /* 成績書き込み  =0 書き込みエラー =0 以外は書き込み成功 */
{
	...
	string a;
	time(&MIKA_lt_t); /* 現在時刻を秒で取得 */
	a=t0conv((long)(MIKA_lt_t-MIKA_st_t+MIKA_rt_t),2); /* 前回の練習時間に今回の練習時間を加算して文字列に変換 */
	err=fprintf(fp,"Total_Practice_Time%s\n",a.c_str()); /* 練習時間を書き込み */
	if(err==0) return(err);
	err=fprintf(fp,"%-34s                ",MIKA_menu_mes_ss[0].c_str()); /* ポジション練習の練習項目名を書き込み */
	if(err==0) return(err);
	a=t0conv(MIKA_p_time,2); /* ポジション練習の累積練習時間を文字列に変換 */
	err=fprintf(fp,format1,a.c_str());	/* ポジション練習の累積練習時間を書き込み */
	if(err==0) return(err);
	for(i=0;i<3;i++) /* ランダム練習成績書き込み */
	{
		err = fprintf(fp, format2, MIKA_menu_mes_ss[i+1].c_str(), MIKA_r_speed[i], MIKA_r_date[i].c_str()); /* 練習項目 最高速度 達成日書き込み */
		if(err==0) return(err);
		a=t0conv(MIKA_r_time[i],2); /* 累積練習時間を文字列に変換 */
...
int wwkiroku(FILE *fp) /* 練習開始時刻と練習時間を成績記録ファイルに書き込む */
{
	...
	if(err==0) return(err);
	err=fprintf(fp," Practice Time%s\n",a.c_str()); /* 練習時間を書き込み */
	return(err);
}
...
//		System.exit(0); /* プログラム終了 */
	...
	void dispretrymessage(HDC g,int flag) /* リトライメッセージ表示 flag=0 表示を行う flag=1 表示を消去 */
	{
		...
		cslput(g,22*16,24*8,MIKA_mesi1); /* 「もう一度練習するときはTABキーを押してください」のメッセージを表示 */
...
// flag=0 表示 flag=1 消去
	{
		...
		if(flag==0)
		{
			...
			offset=0;
			snprintf(a,31,"Current Typing Speed%6.1f/min",MIKA_type_speed); /* 入力速度を文字列に変換 */
		}
		else
		{
			cslcolor(g,MIKA_bk_color);; /* flagが=1の場合は表示消去 */
			offset=20;
			snprintf(a,31,"%6.1f",MIKA_type_speed); /* 入力速度を文字列に変換 */
		}
...
// u_flag=0 練習経過時間秒を2桁の整数で表示 flag=1 練習経過時間秒を小数点以下二桁まで表示
	{
		...
		if(u_flag==0) /* 打ち切りフラグがゼロの場合 */
		{
			if(flag==0)
			{
				snprintf(a,21,"Elapsed Time%2.0fsec",type_speed_time); /* 練習経過時間を整数で表示 */
			}
			else
			{
				if(MIKA_utikiri_flag==1) snprintf(a,21,"%2.0fsec",type_speed_time); /* 練習経過時間を小数点以下二桁まで表示 */
				else
				snprintf(a,21,"%2.0f",type_speed_time); /* 練習経過時間を整数で表示 */
			}
		}
		else /* 打ち切りフラグが1の場合 */
		{
			if(flag==0)
			{
				snprintf(a,21,"Elapsed Time%5.2fsec",type_speed_time); /* 練習経過時間を小数点以下二桁まで表示 */
			}
			else
			{
				if(MIKA_utikiri_flag==1) snprintf(a,21,"%5.2fsec",type_speed_time); /* 練習経過時間を小数点以下二桁まで表示 */
				else
				snprintf(a,21,"%5.2f",type_speed_time); /* 練習経過時間を小数点以下二桁まで表示 */
